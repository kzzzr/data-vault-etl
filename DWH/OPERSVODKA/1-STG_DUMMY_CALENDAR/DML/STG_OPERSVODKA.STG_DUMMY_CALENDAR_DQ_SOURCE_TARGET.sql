...
FROM STG_OPERSVODKA.ETL_LOAD_LOG
	CROSS JOIN (SELECT count(1) AS ROWCOUNT FROM STG_OPERSVODKA.STG_DUMMY_CALENDAR) AS STAGE_COUNT
	CROSS JOIN (SELECT ROWCOUNT FROM STG_OPERSVODKA.ETL_LOAD_LOG
				WHERE OBJECT = 'STG_DUMMY_CALENDAR'
				AND STEP = 'LOAD FILE'
